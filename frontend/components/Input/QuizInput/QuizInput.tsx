...
type QuizInputProps = InputModeProps | ResultModeProps | TeacherModeProps;

const QuizInput: React.FC<QuizInputProps> = (props) => {
  // props 타입 가드 함수
  const isInputMode = (props: QuizInputProps): props is InputModeProps => {
  ...
  return (
    <div className={styles.wrapper}>
      <input
        ...
        onChange={isInputMode(props) ? props.onChange : undefined}
        className={`${styles.input} ${getInputStyles()}`}
        placeholder="정답을 입력해주세요."
        disabled={!isInputMode(props)}
        readOnly={!isInputMode(props)}
      />
